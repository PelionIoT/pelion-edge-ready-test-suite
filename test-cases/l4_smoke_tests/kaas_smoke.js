...
describe("[Level 4] KAASTests", () => {
    describe("#Cluster and node", () => {
        ...
        it('Check cluster status', function(done) {
            exec("kubectl cluster-info", (error, stdout, stderr) => {
                if(error) {
                    ...
                    assert.include(stdout.trim(), "Kubernetes", "Kubernetes cluster is not reachable")
                    assert.include(stdout.trim(), "is running", "Kubernetes cluster is not reachable")
                    done()
                }
            })
        })
        it('Check kubernetes node status', function(done) {
            exec(`kubectl get node ${config.internal_id}`, (error, stdout, stderr) => {
                if(error) {
                    done(error)
                } else {
        ...
        it('Create pod', function(done) {
            ...
            data[2]['spec']['containers'][0]['env'][0]['valueFrom']['configMapKeyRef']['name'] = cm_name
            data[2]['spec']['containers'][0]['env'][1]['valueFrom']['secretKeyRef']['name'] = secret_name
            data[2]['spec']['nodeName'] = config.internal_id
            ...
            setTimeout(() => {
                exec( `kubectl get pod ${pod_name}`, (error, stdout, stderr) => {
                    this.retries(60)
                    sleep(1000)
                    if(error) {
                        done(error)
        ...
        it('Verify pod logs', function(done) {
            exec( `kubectl logs ${pod_name}`, (error, stdout, stderr) => {
                if(error) {
                    done(error)
                } else {
                    assert.include(stdout.trim(), 'pod is running', "Pod is not giving correct logs")
                    done()
                }
            })
        })
